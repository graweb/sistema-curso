    ...
    <form id="formUsuario" class="easyui-form" method="post" data-options="novalidate:true">
        <table style="width:97%;">
            <tr>
                ...
                <td>
                    <select class="easyui-combobox" label="Permissão:" labelPosition="top" id="fkidpermissao" name="fkidpermissao" panelHeight="auto" required="true" style="width:100%;">
                        <?php foreach ($dados_permissao as $per) { 
                            echo "<option value='".$per->idpermissao."'>".$per->nome."</option>";
                        } ?>
                    </select>
                </td>
            </tr>
...
function editarUsuario(){
    ...
    if (row != null){
        ...
        $('#senha').textbox('clear');
        url = '<?php base_url();?>usuarios/atualizar/'+row.idusuario;
    } else {
...
function desativarUsuario(){
    ...
    if (row != null){
        ...
        url = '<?php base_url();?>usuarios/desativar/'+row.idusuario;
    } else {
        $.messager.alert('Atenção','Selecione um registro!','warning');
    }
}
...
function definirSenha(){
    ...
    if (row != null){
        ...
        url = '<?php base_url();?>usuarios/definir_senha/'+row.idusuario;
    } else {
        $.messager.alert('Atenção','Selecione um registro!','warning');
    }
}
