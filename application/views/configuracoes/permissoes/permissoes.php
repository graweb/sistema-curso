    	...
    	<table id="dgPermissoes"
		    ...
		    <thead>
		        <tr>
		            ...
		            <th field="idpermissao" width="10">ID</th>
...
<div id="dlgPermissoesDesativar" class="easyui-dialog" style="width:400px;padding:10px 20px;"
    ...
    <form id="formPermissoesDesativar" class="easyui-form" method="post" data-options="novalidate:true">
        <input type="hidden" id="permissao_ativar_desativar" name="permissao_ativar_desativar">
...
$('#dgPermissoes').datagrid({
    onClickRow: function(index,row){
        ...
        $('#conteudoPermissao').panel('refresh', '<?php base_url();?>permissoes/conteudo_permissao/'+row.idpermissao);
    }
});
...
function editarPermissao(){
    ...
    if (row != null){
        ...
        url = '<?php base_url();?>permissoes/atualizar/'+row.idpermissao;
    } else {
...
function desativarPermissao(){
    ...
    if (row != null){
        if(row.situacao == 1) {
            $('#permissao_ativar_desativar').val(0);
        } else {
            $('#permissao_ativar_desativar').val(1);
        }
        ...
        url = '<?php base_url();?>permissoes/desativar/'+row.idpermissao;
    } else {
        $.messager.alert('Atenção','Selecione um registro!','warning');
    }
}
